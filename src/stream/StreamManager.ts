...
export class StreamManager {
  ...
  /** ダウンロードキュー（同時実行数制限） */
  private static readonly MAX_CONCURRENT_DOWNLOADS = 1;
  ...
  private static readonly LOUDNORM_TARGET = 'I=-14:TP=-1:LRA=11';
  ...
  private async measureLoudness(inputPath: string): Promise<{
   ...
   */
  private async normalizeFile(sourcePath: string): Promise<string> {
   ...
   */
  cleanupCache(extraValidIds: Set<string> = new Set()): {
    tracks: Array<{ id: string; title: string; url: string; cached: boolean; size: number | null }>;
    orphaned: Array<{ id: string; size: number }>;
    deletedCount: number;
    freedBytes: number;
  } {
    // 有効なURLトラックIDを収集（プレイリスト）
    const validIds = new Set<string>(extraValidIds);
    ...
    // 孤立ファイル検出＆削除
    const orphaned: Array<{ id: string; size: number }> = [];
    let freedBytes = 0;
    for (const file of cacheFiles) {
      // file_ プレフィックスはローカルファイル正規化キャッシュ（ハッシュベース管理）
      if (file.startsWith('file_')) continue;
    ...
    const tracks: Array<{ id: string; title: string; url: string; cached: boolean; size: number | null }> = [];
    for (const track of this.tracks) {
      if (track.type === 'url' && track.id) {
